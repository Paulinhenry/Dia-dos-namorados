...
import { Heart, X, Calendar, ChevronDown, Sparkles } from 'lucide-react';

// --- CONFIGURAÇÃO DE CORES E ESTILOS (Injetado via Tailwind ou CSS) ---
// Certifique-se de que as fontes 'Playfair Display' e 'Inter' estão no seu index.html
const styles = {
...
export default function ParaCamyla() {
  ...
  const [timeLeft, setCountdown] = useState({ days: 0, hours: 0, minutes: 0, seconds: 0 });
  ...
  // --- Contador Regressivo (Desde 16 de Dezembro) ---
  useEffect(() => {
    const startDate = new Date('2023-12-16T00:00:00'); // Ajuste o ano correto se necessário

    const updateCountdown = () => {
      const now = new Date();
      const diff = now - startDate; // Tempo decorrido
  ...
  return (
    <div className="min-h-screen text-[#e8eaf6] bg-[#03061a] relative overflow-x-hidden select-none" style={styles.fontInter}>
      ...
      <AnimatePresence>
        {showSplash && (
          ...
          >
            ...
            <p className="text-xs md:text-sm tracking-[0.2em] text-[#7986cb]/80 uppercase mb-10">
              uma história nossa
            </p>
            ...
            <motion.button
              ...
              className="relative px-8 py-4 bg-gradient-to-r from-[#1a237e] to-[#7986cb] rounded-full font-medium tracking-wide shadow-[0_0_30px_rgba(121,134,203,0.4)] hover:shadow-[0_0_50px_rgba(121,134,203,0.7)] transition-all duration-300 group"
            >
              <span className="flex items-center gap-2">
                Abrir com amor <Sparkles size={16} className="text-[#ffd54f]" />
              </span>
            </motion.button>
          </motion.div>
        )}
      ...
      {/* Canvas de Estrelas de Fundo (Apenas renderiza após o splash) */}
      ...
      {!showSplash && (
        <main className="relative z-10">
          ...
          <section className="min-h-screen flex flex-col items-center justify-center px-6 py-20 relative">
            ...
            >
              {/* Substitua pelo link real da sua imagem principal */}
              <img 
                src="https://images.unsplash.com/photo-1518199266791-5375a83190b7?q=80&w=600" 
                alt="Camyla" 
            ...
            <div className="grid grid-cols-2 gap-4 max-w-lg mx-auto">
              {[
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-[#03061a]/80 via-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-fend p-4">
          ...
          <section className="py-24 px-6 text-center bg-gradient-to-b from-transparent via-[#283593]/10 to-transparent">
            ...
            <p className="text-xs text-[#7986cb] tracking-widest uppercase mb-10">Cada segundo ao teu lado conta</p>
          ...
          <section className="py-20 px-6 flex justify-center">
            ...
            >
              ...
              <p className="text-base md:text-lg italic leading-relaxed text-slate-100" style={styles.fontPlayfair}>
                "Desde aquele 16 de dezembro, o meu coração encontrou um porto seguro em ti. Amo-te não apenas pelo que és, mas por quem sou quando estou contigo."
              </p>
      ...
      {/* Lightbox para visualização de fotos em ecrã inteiro */}
      <AnimatePresence>
        {lightboxImg && (
          <motion.div 
            initial={{ opacity: 0 }}
          ...
          >
            <button 
              onClick={() => setLightboxImg(null)}
              className="absolute top-6 right-6 bg-white/10 hover:bg-white/20 text-white rounded-full p-3 transition-colors duration-200"
            >
              <X size={20} />
            </button>
            <motion.img 
